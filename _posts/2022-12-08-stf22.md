...
We are interested in using this payload to redirect the same bot to dump its flag cookie into our cookie stealing server. With some guidance from [John Hammond](https://www.youtube.com/watch?v=uU_tvQPCBUo&ab_channel=JohnHammond), let's first cut away the alerts in the payload and apply the same Blog Post 1 JavaScript function in **eval** on line 7.
